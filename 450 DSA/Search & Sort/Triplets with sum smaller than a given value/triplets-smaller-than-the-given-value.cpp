long long countTriplets(long long arr[], int n, long long   k)
	{
	   ...
	   sort(arr,arr+n);
	   long long count = 0; /// initialised counter to 0
    ...
    /// Three Pointer Approach
	   ...
	   for(int i=0;i<n;i++){
	       ...
	       while(s<e){
	           if(arr[i]+arr[s]+arr[e]>=k) e--; /// if the sum is greater than the expected sum then we are decreasing our third pointer by 1
