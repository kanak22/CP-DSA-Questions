...
int minimumNumber(int n, string password) {
    ...
    for(i=0;i<4;i++){
        if(arr[i]==0) count++;    //if there is a missing character, count it
    }

    int x= 6-(n+count);    //if the length is not sufficient, take it in variable x

    if(x>0) count+=x;    //add the additional characters required

    return count;     //return the additional characters required for valid password
}
